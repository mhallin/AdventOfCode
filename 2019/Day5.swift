...
class IntCodeMachine {
    private static let operations: [Int: (IntCodeMachine) -> (Int) -> StopReason?] = [
        1: operationAdd,
        2: operationMultiply,
        3: operationInput,
        4: operationOutput,
    ...
    private func operationAdd(opcode: Int) -> StopReason? {
    ...
    private func operationMultiply(opcode: Int) -> StopReason? {
        let lhs = self.fetch(ispOffset: 1, mode: opcode / 100)
        let rhs = self.fetch(ispOffset: 2, mode: opcode / 1000)
        let resultPtr = self.memory[self.isp + 3]
        self.isp += 4
    ...
    private func operationInput(opcode: Int) -> StopReason? {
    ...
    private func operationOutput(opcode: Int) -> StopReason? {
        let result = self.fetch(ispOffset: 1, mode: opcode / 100)
        self.isp += 2
        return .output(result)
    }

    private func operationJumpIfTrue(opcode: Int) -> StopReason? {
    ...
    private func operationJumpIfFalse(opcode: Int) -> StopReason? {
        let condition = self.fetch(ispOffset: 1, mode: opcode / 100)
        let destination = self.fetch(ispOffset: 2, mode: opcode / 1000)
    ...
    private func operationLessThan(opcode: Int) -> StopReason? {
        let lhs = self.fetch(ispOffset: 1, mode: opcode / 100)
        let rhs = self.fetch(ispOffset: 2, mode: opcode / 1000)
        let resultPtr = self.memory[self.isp + 3]
    ...
    private func operationEqual(opcode: Int) -> StopReason? {
        let lhs = self.fetch(ispOffset: 1, mode: opcode / 100)
        let rhs = self.fetch(ispOffset: 2, mode: opcode / 1000)
        let resultPtr = self.memory[self.isp + 3]
    ...
    private func operationHalt(opcode: Int) -> StopReason? {
        return .halted
    }
}
